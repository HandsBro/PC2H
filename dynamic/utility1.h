...
public:
	unsigned int m_size;
	_T* m_data;//concrete int list serve as vectors.
	unsigned int m_num;//index of the last effective item + 1 (empty and ready to receive)
...
//		printf("iVector allocate: %d\n",n);
		m_size = n;//maximum size
		m_data = new _T[m_size];
		m_num = 0;//effective pointer recorded.
	}
	void push_back( _T d )
	{
		if ( m_num == m_size )
...
//! occur is used to store effective positied used in m_data since not all are used. Ans m_data is used as map where index is the key and content is the map.
struct dMap
{	
	double* m_data;
	int m_num;
